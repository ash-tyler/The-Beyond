Shader "Enviro/RaymarchClouds"
{
	...
	float4 GetHeightGradient(float cloudType)
	{
		...
		const float4 CloudGradient3 = float4(0.0, 0.1, 0.75, 0.98);
	...
	float CalculateCloudDensity(float3 pos, float height, float3 weather, float dist, bool details)
	{
		...
		float4 coord = float4(pos * baseFreq * (_BaseNoiseUV*dist), 0.0);
		coord.xyz += float3(_CloudsAnimation.x * 20, -_Time.x * 0.15 ,_CloudsAnimation.y * 20);

		float4 baseNoise = tex3Dlod(_Noise, coord);
		...
		cloud_coverage = pow(cloud_coverage, Remap(height, 0.7, 0.8, 1.0, lerp(1.0, 0.5, 1)));
		...
		if (details)
		{
			coord = float4(pos * baseFreq * (_DetailNoiseUV*dist), 0.0);
			coord.xyz += float3(_CloudsAnimation.x * 20, -_Time.x * 0.15, _CloudsAnimation.y * 20);
			...
			float high_freq_noise_modifier = lerp(high_freq_fBm, 1.0f - high_freq_fBm, saturate(height * 10.0));
			cloudDensity = Remap(cloudDensity, high_freq_noise_modifier * 0.2, 1.0, 0.0, 1.0);
		}
		///
	...
	{
		float sunRayStepLength = (_CloudsParameter.z * 0.75) / 5;
		float3 sunRayStep = 0;
	...
	float4 frag(v2f i) : SV_Target
	{
		...
		float4 sky = ComputeScatteringClouds(viewDir, i.sky.xy);
		...
		// Reduce base and detail noise texture tiling at distance clouds to improve performance.
#if ENVIRO_CLOUD_FAR_LOD
		//float dist = clamp(ray.y * 4,0.25,1);
		float dist = clamp(saturate((3 * dot(normalize(wsDir), float3(0, 2, 0)))), 0.1, 1);
#else
		float dist = 1;
#endif
		//Raymarching
		...
		for (int i = 0; i < executeSteps; i++)
		{
			...
			cloudDensity = CalculateCloudDensity(pos, height, weather, dist, false);

			[branch]
			if (cloudDensity > 0.0)
			{
				float3 ds = GetDensityAlongRay(pos, pCent, LightDirection, weather, dist, height);
